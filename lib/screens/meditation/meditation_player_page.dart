...
class _MeditationPlayerPageState extends State<MeditationPlayerPage> {
  ...
  Future<void> _load() async {
    ...
    if (_t == null) return;
    await _player.setAudioSource(AudioSource.uri(Uri.parse('asset://${_t!.asset}')));
    await _player.setLoopMode(LoopMode.one);
    await _player.setVolume(.8);
    _player.play(); // sofort starten
    if (mounted) setState((){});
  }
  ...
  Widget build(BuildContext context) {
    ...
    return CupertinoPageScaffold(
      ...
      child: t == null ? const Center(child: CupertinoActivityIndicator()) : SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text(t.title, style: const TextStyle(color: _white, fontSize: 22, fontWeight: FontWeight.w800)),
              ...
              builder: (_, snap) {
                ...
                return Row(
                  ...
                  children: [
                    ...
                    CupertinoButton(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                      onPressed: () async { final v = await _player.volume; _player.setVolume((v+0.1).clamp(0,1)); },
            ...
            StreamBuilder<Duration>(
              ...
              builder: (_, snap) {
                ...
                return Column(
                  children: [
                    CupertinoSlider(value: p.clamp(0.0, 1.0), onChanged: (v) {
            ...
            ValueListenableBuilder<LoopMode>(
              valueListenable: _player.loopModeStream.asBroadcastStream().asBroadcastStream().map((e)=>e).asBroadcastStream().toValueNotifier(LoopMode.one),
              builder: (_, mode, __) {
                final on = mode == LoopMode.one;
                return CupertinoListTile(
                  backgroundColor: const Color(0x11000000),
                  title: const Text('Endlos wiederholen', style: TextStyle(color: _white)),
...
// kleines Hilfs-Ext, um Stream in ValueListenable nutzen zu können
extension _ToValueNotifier<T> on Stream<T> {
  ValueNotifier<T> toValueNotifier(T initial) {
    final n = ValueNotifier(initial);
    listen((v) => n.value = v);
    return n;
  }
}
